...
function init (env, ctx) {
  ...
  function getAlarm (level, group) {
    ...
    if (!alarm) {
      var display = group === 'default' ? levels.toDisplay(level) : group + ':' + level;
  ...
  function emitNotification (notify) {
    ...
    if (ctx.ddata.lastUpdated > alarm.lastAckTime + alarm.silenceTime) {
      ...
      /* ODH - Alarm creation area */

      // Get notification times
      if (env.settings.CUSTOMER_ALARMS_CALL_TIMES !== null) {
        var timesToCall = env.settings.CUSTOMER_ALARMS_CALL_TIMES.split('-');
        var issueCallAlarm = false;

        console.log('ODH Start: ' + timesToCall[0]);
        console.log('ODH Start: ' + timesToCall[1]);
        var d = new Date();
        var callStartTime = new Date(d.getFullYear(), d.getMonth(), d.getDate(), parseInt(timesToCall[0]), 0, 0, 0).getTime();
        var callEndTime = new Date(d.getFullYear(), d.getMonth(), d.getDate(), parseInt(timesToCall[1]), 0, 0, 0).getTime();

        console.log('ODH Start: ' + callStartTime);
        console.log('ODH Start: ' + d.getTime());
        console.log('ODH End  : ' + callEndTime);

        if (d.getTime() >= callStartTime && callEndTime > d.getTime()) {
          issueCallAlarm = true;
        }
      }
      else {
      ...
      if (notify.level == 'Urgent' && issueCallAlarm) {
        // Make Twilio Alert / Call.
        const accountSid = 'AC9b060ef900eba2ec600b75f7a2e6f590';
        const authToken = 'a2ae957d0c61b9693ee372c4f9f61912';
        const twClient = require('twilio')(accountSid, authToken);

        twClient.calls.create({
              url: 'https://oliverhelm.me/otto/pump-alert.xml',
              to: env.settings.CUSTOMER_ALARMS_CALL_NUMBER,
              from: '+443330164104'
  ...
  notifications.ack = function ack (level, group, time, sendClear) {
    ...
    if (sendClear) {
      var notify = {
        clear: true
        , title: 'All Clear'
        , message: group + ' - ' + levels.toDisplay(level) + ' was ack\'d'
        , group: group
      };
      ctx.bus.emit('notification', notify);
      logEmitEvent(notify);
